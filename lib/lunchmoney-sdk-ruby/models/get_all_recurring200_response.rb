...
module LunchMoney
  class GetAllRecurring200Response < ApiModelBase
    attr_accessor :recurring_items
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LunchMoney::GetAllRecurring200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LunchMoney::GetAllRecurring200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'recurring_items')
        if (value = attributes[:'recurring_items']).is_a?(Array)
          self.recurring_items = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          recurring_items == o.recurring_items
    end
