...
module LunchMoney
  class GetAllPlaidAccounts200Response < ApiModelBase
    attr_accessor :plaid_accounts
    ...
    def self.openapi_types
      {
        :'plaid_accounts' => :'Array<PlaidAccountObject>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LunchMoney::GetAllPlaidAccounts200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LunchMoney::GetAllPlaidAccounts200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'plaid_accounts')
        if (value = attributes[:'plaid_accounts']).is_a?(Array)
          self.plaid_accounts = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          plaid_accounts == o.plaid_accounts
    end
