...
module LunchMoney
  class DeleteCategoryResponseWithDependencies < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LunchMoney::DeleteCategoryResponseWithDependencies` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LunchMoney::DeleteCategoryResponseWithDependencies`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @dependents.nil?
        invalid_properties.push('invalid value for "dependents", dependents cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @dependents.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] dependents Value to be assigned
    def dependents=(dependents)
      if dependents.nil?
        fail ArgumentError, 'dependents cannot be nil'
      end
