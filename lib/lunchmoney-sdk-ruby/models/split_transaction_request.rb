...
module LunchMoney
  class SplitTransactionRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        :'child_transactions' => :'Array<SplitTransactionObject>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LunchMoney::SplitTransactionRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LunchMoney::SplitTransactionRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @child_transactions.nil?
        invalid_properties.push('invalid value for "child_transactions", child_transactions cannot be nil.')
      end

      if @child_transactions.length > 500
        invalid_properties.push('invalid value for "child_transactions", number of items must be less than or equal to 500.')
      end

      if @child_transactions.length < 2
        invalid_properties.push('invalid value for "child_transactions", number of items must be greater than or equal to 2.')
      end
    ...
    def valid?
      ...
      return false if @child_transactions.nil?
      return false if @child_transactions.length > 500
      return false if @child_transactions.length < 2
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] child_transactions Value to be assigned
    def child_transactions=(child_transactions)
      if child_transactions.nil?
        fail ArgumentError, 'child_transactions cannot be nil'
      end

      if child_transactions.length > 500
        fail ArgumentError, 'invalid value for "child_transactions", number of items must be less than or equal to 500.'
      end

      if child_transactions.length < 2
        fail ArgumentError, 'invalid value for "child_transactions", number of items must be greater than or equal to 2.'
      end
