...
module LunchMoney
  class TransactionObject < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LunchMoney::TransactionObject` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LunchMoney::TransactionObject`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, date, amount, currency, to_base, recurring_id, payee, original_name, category_id, plaid_account_id, manual_account_id, external_id, tag_ids, notes, status, is_pending, created_at, updated_at, is_split_parent, split_parent_id, is_group_parent, group_parent_id, children, plaid_metadata, custom_metadata, files, source].hash
    end
